...
export class GameTopPlayersComponent implements OnChanges {
   ...
   *
   * Skaters are picked on their rating plus the star boost (StatsUtils.sortByBoostedRating), so a star takes the
   * spot from a teammate rated up to half a point better than him, and two who sort the same are split by how big
   * a star they are. The rating on every card is still the player's own. No goalie is a star, so they are picked
   * on their rating and time on ice alone.
   */
  private getSpots(players: GamePlayer[]): TopPlayerSpot[] {
    const sortedPlayers = [...(players ?? [])].sort((playerA, playerB) => StatsUtils.sortByBoostedRating(playerA, playerB) ||
