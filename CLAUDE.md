...
- `HokMob.App/Controllers/NhlController.cs`: proxies `/api/nhl/<path>` → `https://api-web.nhle.com/v1/<path>`. Only
  ...
  trailing `/` is fine.
- `HokMob.App/ClientApp/src/assets/history/`: the history page's season files, generated by
  `ClientApp/build-season-data.js` from the stats API (see "History data" in `docs/nhl-api.md`). Served as static files,
  so no controller is involved.
...
- `ClientApp/src/app/shared/`:
  - `models/nhl-web-api/`: typed models for api-web and the player search. `models/nhl-stats-api/`: the
    `/api/nhl-stats/*` responses. `models/nhl-history/`: the history page's season files and rated games.
...
- A finished season's history file: `npm run build-season-data -- 20252026` (from `HokMob.App/ClientApp`), then add
  the season to `SeasonHistoryService.seasons`. See "History data" in `docs/nhl-api.md`.
